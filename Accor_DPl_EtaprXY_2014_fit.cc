...
{
  Double_t value = 1.+ 2.*par[0]*y + (par[0]*par[0]+par[1]*par[1])*y*y + par[2]*x*x;
 return value;
}
...
{
 Double_t value = 1.+ par[0]*y + par[1]*y*y + par[2]*x*x + par[3]*x*x*y;
 return value;
}
...
{
 Double_t value = 1.+ par[0]*y + par[1]*y*y + par[2]*x*x + par[3]*x*x*y + par[4]*x*x*x*x;
 return value;
}
...
{
  //Double_t value = 1.+ 2.*par[0]*y + (par[0]*par[0]+par[1]*par[1])*y*y + par[2]*x*x + par[3]*x*x*y;
  //Double_t value = 1.+ par[0]*y + par[1]*y*y + par[2]*x*x + par[3]*fabs(x);
  Double_t value = 1.+ par[0]*y + par[1]*y*y + par[2]*x*x + par[3]*x*x*x*x;
 return value;
}
//______________________________________________________________________________
...
void fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
  ...
  for ( iev=0; iev<nmc; iev++ ) {
    xMC = MCX[iev];
    yMC = MCY[iev];
    if ( iflnfun==0 ) wMCi = func1(xMC,yMC,par);
    else if ( iflnfun==2 ) wMCi = func3(xMC,yMC,par);
    else if ( iflnfun==3 ) wMCi = func4(xMC,yMC,par);
    else if ( iflnfun==4 ) wMCi = func5(xMC,yMC,par);
    else if ( iflnfun==5 ) wMCi = func6(xMC,yMC,par);
    else              wMCi = func2(xMC,yMC,par);
  ...
  for ( ix=1; ix<=nhx; ix++ ) {
	...
	if ( wdat/edat < rerr ) {
      ...
      wsim = hxy_wmc->GetBinContent(ix,iy);
      if ( iflnfun!=5 ) wsim *= wNorm;
      etot2 = edat*edat;
      if ( if_mchi2 != 0 ) {
	esim = hxy_wmc->GetBinError(ix,iy);
	if ( iflnfun!=5 ) esim *= wNorm;
	etot2 += esim*esim;
      }
      dat_sim = wdat - wsim;
  ...
  if ( nftpr==3 ) printf("%d %f %f %f %f\n",ntbin,chisq,par[0],par[1],par[2]);
  if ( nftpr==4 ) printf("%d %f %f %f %f %f\n",ntbin,chisq,par[0],par[1],par[2],par[3]);
  if ( nftpr==5 ) printf("%d %f %f %f %f %f %f\n",ntbin,chisq,par[0],par[1],par[2],par[3],par[4]);
  f = chisq;
}

//--------------------------------------------------------------------------

void Accor_DPl_EtaprXY_2014_fit( Double_t a=-0.07, Double_t b=-0.07, Double_t d=-0.06,
				 Double_t we=0.002, Int_t iffit=1, Int_t iflin=0, Int_t ifile=0,
			   Int_t lmc=0,  Int_t kfit=2, Double_t wup=1., Double_t rer=4. )
{
  char *FileName,*FileName2,*FileName3,*FileName4,*FileName5,*FileName6;
  FileName = "DalPl_etapr_eta2pi0_data_accor_v2_read.dat";
  if ( ifile==1 ) FileName = "DalPl_etapr_eta2pi0_data_accor_v3_read.dat";
  ...
  iflnfun=iflin;
  Int_t nfitpr=3;
  if ( iflnfun==2 ) nfitpr=4;
  if ( iflnfun==3 ) nfitpr=5;
  if ( iflnfun==4 ) nfitpr=4;
  if ( iflnfun==5 ) nfitpr=4;
  nftpr = nfitpr;
  ...
  vstart[ipar++] = b;
  vstart[ipar++] = d;
  if ( nfitpr>4 ) vstart[ipar++] = c;
  if ( nfitpr>5 ) vstart[ipar++] = 0.;
  if ( iflnfun==5 ) vstart[ipar++] = rdtmc;
  ...
  for ( Int_t ip=0; ip<=2; ip++ ) {
    if ( ip==0 ) {
      ...
      fscanf(f_dpl,"%d%lf%lf%d%lf%lf",&nhix,&xmin,&xmax,&nhiy,&ymin,&ymax);
      printf("Dal.pl: %d %f %f %d %f %f\n",nhix,xmin,xmax,nhiy,ymin,ymax);
      nhx = nhix;
      nhy = nhiy;
      hxy_data = new TH2F("hxy_data","hxy_data",nhx,xmin,xmax,nhy,ymin,ymax);
	...
	fscanf(f_dpl,"%d%d%lf%lf%lf%lf%lf%lf",&ix,&iy,&x,&y,&xMC,&yMC,&wdat,&edat);
    ...
    if ( ip==1 ) {
      ...
      hxy_wmc = new TH2F("hxy_wmc","hxy_wmc",nhx,xmin,xmax,nhy,ymin,ymax);
      hxy_wmc->Sumw2();
	...
	for ( iev=0; iev<200000; iev++ ) {
	  ...
	  fscanf(f_dk,"%lf%lf%lf%lf%lf",&dk_x,&dk_y,&dk_m2pi0,&dk_mpi0eta[0],&dk_mpi0eta[1]);
	  xMC = fabs(dk_x);
	  yMC = dk_y;
	  if ( iflnfun==0 ) wMCi = func1(xMC,yMC,par4fit);
	  else if ( iflnfun==2 ) wMCi = func3(xMC,yMC,par4fit);
	  else if ( iflnfun==3 ) wMCi = func4(xMC,yMC,par4fit);
	  else if ( iflnfun==4 ) wMCi = func5(xMC,yMC,par4fit);
	  else              wMCi = func2(xMC,yMC,par4fit);
 	  hxy_mc -> Fill(xMC,yMC);
 	  hxy_wmc -> Fill(xMC,yMC,wMCi);
	  MCX[nmc] = xMC;
	  MCY[nmc] = yMC;
	...
	arglist[0] = 0;
	arglist[1] = wup;
	if ( kfit==0 ) miniXY->mnexcm("CALL FCN", arglist ,2,ierflg);
	else if ( kfit==1 ) {
	  arglist[1] *= 0.1;
